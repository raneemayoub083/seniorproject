    ...
    <div class="container">
        <p class="mt-4" style="color:#729762;text-align:center;text-shadow:2px 2px 5px white;">
            <button data-text="Awesome" class="buttonpma">
                <span class="actual-text">&nbsp;Applications&nbsp;List&nbsp;</span>
                <span class="hover-text" aria-hidden="true">&nbsp;Applications&nbsp;List&nbsp;</span>
            </button>
        </p>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <img src="{{ Storage::url($application->profile_img) }}" alt="Profile Image" width="50">
                            @else
                            N/A
                            @endif
                        </td>
                        ...
                        <td>{{ $application->parents_contact_numbers }}</td>

                        <td>
                            <button class="shadow__btn">
                                @if($application->id_card_img)
                                <a href="{{ Storage::url($application->id_card_img) }}" target="_blank">View</a>
                                @else
                                N/A
                                @endif
                            </button>
                        </td>
                        <td><a href="#" onclick="handleDocument('{{ Storage::url($application->precertificate) }}'); return false;">
                        ...
                        <td>
                            @foreach($application->disabilities as $disability)
                        ...
                        <td>
                            ...
                            <br>
                            <!-- Approve Button -->
                            <button class="btn btn-success" onclick="confirmApprove('{{ $application->id }}')">Approve</button>
                        ...
                        <td>
                            <a href="{{ route('application.edit', ['id' => $application->id]) }}" class="btn btn-warning">Edit</a>
                            <button class="btn btn-danger" onclick="confirmDelete('{{ $application->id }}')">Delete</button>
                            <form id="delete-form-{{ $application->id }}" action="{{ route('application.destroy', ['id' => $application->id]) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
    ...
    function confirmApprove(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = "{{ route('application.updateStatus', ['id' => ':id', 'status' => 'approved']) }}".replace(':id', id);
    ...
    function confirmReject(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                var form = document.createElement('form');
                form.method = 'POST';
                form.action = "{{ route('application.updateStatus', ['id' => ':id', 'status' => 'rejected']) }}".replace(':id', id);
